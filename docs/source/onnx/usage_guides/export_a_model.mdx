...
## Int32 Input Conversion

Some ONNX runtimes and hardware accelerators work better with int32 inputs rather than int64 inputs. Optimum provides an `Int32Wrapper` that automatically converts `input_ids` from int64 (torch.long) to int32 during export.

To enable int32 input conversion, set `use_int32_inputs=True` in your ONNX configuration:
...
# Export with int32 conversion enabled
onnx_config = MyOnnxConfig(model.config, use_int32_inputs=True)
```

The `Int32Wrapper` will transparently convert int64 `input_ids` to int32 during the forward pass, ensuring better compatibility with ONNX runtimes that prefer int32 inputs while maintaining the same model behavior.
